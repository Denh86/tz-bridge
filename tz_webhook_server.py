...
from QuantConnect.Brokerages import InteractiveBrokersBrokerageModel
import threading
...
class EODGainerShort(QCAlgorithm):
    def Initialize(self):
        ...
        self.limit_buffer_pct   = 0.005
        self.exit_limit_buffer  = 0.005
        self.force_close_buffer = 0.010
        ...
        # OnWarmupFinished triggers the scan+entry immediately after warmup.
        # For subsequent days, scan fires at 4:06 PM and entry at 4:08 PM.
        self.Schedule.On(self.DateRules.EveryDay(), self.TimeRules.At(16, 6),  self.ScanAndEnter)
    ...
    def _fire_webhook(self, action, symbol, quantity, price):
        ...
        """
        # Strip QC SID suffix e.g. "DLXY YTYSIFTLVN8L" → "DLXY"
    ...
    # ============================================================================
    # WARMUP FINISHED — scan immediately if we are already in AH window
    # ============================================================================
    def OnWarmupFinished(self):
        self.Log(f"OnWarmupFinished at {self.Time}")
        now = self.Time.time()
        # If warmup finished during or after 4:00 PM AH window, scan and enter now
        if time(16, 0) <= now <= time(19, 25):
            self.Log("Warmup finished during AH window — scanning immediately")
            self.ScanAndEnter()
        else:
            self.Log(f"Warmup finished outside AH window ({now}) — waiting for 4:06 PM schedule")

    # ============================================================================
    # COMBINED SCAN + ENTER (called from schedule at 4:06 PM or immediately post-warmup)
    # ============================================================================
    def ScanAndEnter(self):
        if self.IsWarmingUp or self.IsWeekend():
            return
        if self.entry_executed:
            self.Log("ScanAndEnter: entry already executed today, skipping")
            return
        self.ScanDayGainers()
        if self.top_gainer_symbol:
            self.PlaceEntryOrder()
    ...
    def ScanDayGainers(self):
        ...
        # RTH volume window: full 9:30-4:00 session for reliable dollar volume
        rth_start  = self.nyse_tz.localize(
            datetime(today.year, today.month, today.day, 9, 30))
        rth_end    = self.nyse_tz.localize(
            datetime(today.year, today.month, today.day, 16, 0))

        self.Log(f"DAY SCAN: {self.Time.strftime('%H:%M')} snapshot for {today} | "
        ...
        for i in range(0, len(self.liquid_universe_symbols), batch_size):
            for symbol in self.liquid_universe_symbols[i : i + batch_size]:
                try:
                    ...
                    # Current price — use AH or most recent available
                    sec = self.Securities.get(symbol)
                    current_price = float(sec.Price) if sec and sec.Price > 0 else 0
                    if current_price <= 0:
                        stats['no_rth'] += 1
                        continue

                    # Full RTH dollar volume (9:30-4:00) — avoids tiny AH volume rejections
                    rth = self.History(
                        [symbol], rth_start, rth_end,
    ...
    def PlaceEntryOrder(self):
        ...
        self._webhook_cover_fired = False
        self._fire_webhook("SHORT", symbol.Value, quantity, current_price)
        ...
        if current_price <= tp_trigger:
            ...
            if qty > 0:
                ...
                if self._webhook_short_fired and not self._webhook_cover_fired:
                    ...
                    self._fire_webhook("COVER", symbol.Value, qty, current_price)
            return
    ...
    def ForceClosePositions(self):
        ...
        for holding in self.Portfolio.Values:
            ...
            if self.exit_triggered:
                self.Log(f"  SKIP: {symbol} exit already triggered")
                continue
            if current_price <= 0:
        ...
        for symbol in list(self.entry_tickets.keys()):
            ...
            if self._webhook_short_fired and not self._webhook_cover_fired:
                self._webhook_cover_fired = True
                self._fire_webhook("CANCEL", symbol.Value, 0, 0)
            self._CancelExitOrder(symbol)
            self._Cleanup(symbol)
